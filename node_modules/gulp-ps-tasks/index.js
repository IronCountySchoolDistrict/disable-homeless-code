...
exports.default = function (gulp, projectPath) {
  ...
  gulp.task('deploy', () => gulp.src('dist/src/**').pipe(deploy()));
  ...
  gulp.task('build', ['build:less', 'build:sass', 'build:static', 'build:babel'], () => gulp.src(['./plugin/**/*', './src/**/*', './queries_root/**/*', 'plugin/plugin.xml', '!src/**/*.less', '!src/**/.*scss', '!src/**/*.{png,gif,jpg,bmp,swf,js}', '!src/**/ext/**', '!src/**/less{,/**}', '!src/**/sass{,/**}', '!plugin/web_root/admin/**/*.js', '!plugin/WEB_ROOT/admin/**/*.js'], {
    base: './'
  }).pipe(preprocess()).pipe(gulp.dest('dist')));
  ...
  gulp.task('build:babel', () => gulp.src(['./src/**/*.js', '!src/**/ext/**'], {
    base: './'
  }).pipe(preprocess()).pipe(plugins.babel({
    plugins: ['transform-es2015-modules-amd', 'transform-es2015-classes']
  })).pipe(gulp.dest('dist')));
  ...
  gulp.task('build:lint', () => gulp.src('src/**/*.js').pipe(plugins.eslint()).pipe(plugins.eslint.format()));

  gulp.task('build:less', () => gulp.src(['src/**/*.less', '!src/**/ext/**', '!src/**/less/**']).pipe(plugins.less()).pipe(preprocess()).pipe(gulp.dest('dist/src')));

  gulp.task('build:sass', () => gulp.src(['src/scripts/*', '!src/**/ext/**']).pipe(plugins.flatmap((stream, dir) => gulp.src(dir.path + '/**/*.scss').pipe(plugins.sass().on('error', plugins.sass.logError)).pipe(plugins.concatCss('css/bundle.css')).pipe(gulp.dest('dist/src/scripts/' + (0, _path.basename)(dir.path))))));
};
